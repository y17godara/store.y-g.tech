...
export function Grid({
  ...
  return (
    <>
      <li className='border-tertiary flex min-h-32 flex-col gap-4 rounded-md border px-2 py-4'>
...
function ViewGrid({ products }: { products: Product[] }): JSX.Element {
  ...
  return (
    <>
      <ul className='animate-list grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
        {currentProducts.map((product) => (
          <Grid key={product.id} {...product} />
        ))}
      </ul>
